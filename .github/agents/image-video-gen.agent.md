...
6. **Style determination** — if the user specifies an art style, use it verbatim. If not, auto-select based on topic:
...
7. **Language & voice detection** — Determine the user's requested language:
...
For each scene, invoke the `pollinations-python` skill to generate the image using the prompt from Phase 4, passing `{image_model}` as the model.

Before generating each image, check if `{project_folder}/scene_{x}/image_{x}.jpeg` already exists. If it does, **skip that scene** (resumable pipeline).

Save each generated image to:
```
{project_folder}/scene_{x}/image_{x}.jpeg
```

Generation parameters:
- `model` = `{image_model}`
- `width` = `1080`, `height` = `1920` (vertical short-form)
- `seed` = `x * 42` (scene number × 42 for reproducibility)
- The API returns JPEG binary — save with `.jpeg` extension

### Phase 5.5 — TTS Audio Generation per Scene

For each scene, invoke the `edge-tts` skill to generate narration audio and a word-level SRT.

**Before generating**, check if `{project_folder}/scene_{x}/audio_{x}.mp3` already exists — skip if so (resumable).

For each scene:
1. Read `{project_folder}/scene_{x}/subtitles_{x}.txt`
2. Generate TTS audio + word-level SRT using:
   - voice: `{EDGE_TTS_NAME}` from `.env`
   - rate/volume/pitch: default `+0%` unless user overrides
3. Save to:
   ```
   {project_folder}/scene_{x}/audio_{x}.mp3
   {project_folder}/scene_{x}/subtitle_{x}.srt
   ```
4. The word-level SRT from edge-tts gives per-word timing. Post-process it to group words into blocks of `{WORD_BREAK_SUBTITLE}` words:
   - If `WORD_BREAK_SUBTITLE=4`: group every 4 words into one SRT entry, using the start time of the first word and end time of the last word in the group
   - Save the grouped SRT (overwrite `subtitle_{x}.srt` with the grouped version)

**After all scenes are done:**
1. Concatenate all per-scene audio files into a single full narration:
   ```
   {project_folder}/audio_full.mp3
   ```
   Use FFmpeg concat or simple binary join (same codec).

2. Merge all per-scene SRTs into one continuous `subtitles.srt` with timestamps offset by cumulative scene duration. This replaces the estimated SRT from Phase 2 with **actual TTS timing**.
...
**BGM Selection:**
1. Analyse the video topic, mood, and `{image_style}` to determine the appropriate music mood
2. Search Pixabay for matching tracks using the Audio Selection Strategy from the `pixabay-audio` skill
3. Download the best-matching track to:
   ```
   {project_folder}/audio/bgm_{slug}.mp3
   ```
4. If a suitable BGM already exists in `{project_folder}/audio/`, reuse it (resumable)

**SFX Selection (optional):**
1. For each scene, analyse the prompt and narration to determine if an SFX would enhance the scene (e.g. whoosh for transitions, ambient for nature scenes)
2. Not every scene needs SFX — only add when it genuinely enhances the experience
3. Download matching SFX to:
   ```
   {project_folder}/audio/sfx_{name}.mp3
   ```

**Save an audio manifest** to `{project_folder}/audio/audio_manifest.json` with:
- BGM: file name, Pixabay ID, query used, duration, assignment
- SFX: file name, Pixabay ID, query used, assigned scenes
